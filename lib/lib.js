...
function boardDestroy(){
  var boardNode = document.getElementsByClassName('board')[0];
  var boardNodeChildren = boardNode.childNodes
  //console.log("nodeList Length: " + boardNodeChildren.length);
  
  for(let i = 0; i < boardNodeChildren.length; i++){
    
    //console.log(boardNodeChildren[i]);
    boardNode.removeChild(boardNodeChildren[i]);
    i--;
   
    
  }
 // console.log(boardNode.childNodes);
...
function showCell (evt) {
  ...
  if (evt.target.classList.contains('mine')) {
    displayMessage('BOOM! Teemo has murdered you with his mushrooms.')

    let randomNum = Math.floor(Math.random()*4);

    console.log(randomNum);
    
    document.getElementById("lose"+ randomNum ).play()
